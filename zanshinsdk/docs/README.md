...
Onboarding is the process of adding new Amazon Web Services (AWS) Account (Scan Targets) to your Organization in Zanshin.
...
This method automatically creates a new Scan Targets to the Zanshin Organization informed in parameters and performs the onboard creating the roles required in your Amazon Web Services (AWS) Account.

Currently using the Zanshin SDK you're only able to onboard **Amazon Web Services (AWS) Scan Targets**.

> :warning: This method will deploy a CloudFormation stack in your Amazon Web Services (AWS) account. So it's required that you have `boto3` installed in your system, as well as privileges to create CloudFormation stacks and create IAM roles in the given Amazon Web Services (AWS) Account.
...
**Currently supports only Amazon Web Services (AWS) Scan Targets.**

_For Amazon Web Services (AWS) Scan Target:_

If boto3 is installed, creates a Scan Target for the given organization and perform the onboard.

Parameters:

- :param region: the Amazon Web Services (AWS) Region to deploy the CloudFormation Template of Zanshin Service Role.
- :param organization_id: the ID of the organization to have the new Scan Target.
- :param kind: the Kind of scan target (AWS / Amazon Web Services, GCP / Google Cloud Platform, AZURE / Microsoft Azure, DOMAIN)
...
#### Minimum required Amazon Web Services (AWS) IAM Privileges
...
```json
{
	...
	"Statement": [{
		...
		},
		{
			...
			"Effect": "Allow",
			"Resource": "arn:aws:iam::<your Amazon Web Services (AWS) account ID|*>:role/Tenchi-Zanshin-Service-Role",
			"Condition": {
...
> **Attention**
> :warning: Make sure to substitute the `your Amazon Web Services (AWS) account ID` to the correct value.
